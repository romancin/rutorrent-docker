      ...
      steps {
        script {
          ...
          docker.withRegistry('', registryCredential) {
            def image = docker.build("$registry:$gitbranch",  "--build-arg BASEIMAGE_VERSION=3.12 --build-arg RTORRENT_VER=v0.9.8 --build-arg LIBTORRENT_VER=v0.13.8 --build-arg MAXMIND_LICENSE_KEY=${MAXMIND_LICENSE_KEY} -f Dockerfile .")
            image.push()
            image.push(base)
            image.push(major)
            image.push(minor)
    ...
    stage('Building image and pushing it to the registry (master)') {
      ...
      steps {
        script {
          ...
          docker.withRegistry('', registryCredential) {
            def image = docker.build("$registry:latest", "--build-arg BASEIMAGE_VERSION=3.12 --build-arg RTORRENT_VER=v0.9.8 --build-arg LIBTORRENT_VER=v0.13.8 --build-arg MAXMIND_LICENSE_KEY=${MAXMIND_LICENSE_KEY} -f Dockerfile .")
            image.push()
            image.push(base)
            image.push(major)
            image.push(minor)
